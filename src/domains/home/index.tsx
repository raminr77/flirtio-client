    ...
    <div className='flex flex-col w-full justify-center items-center relative'>
      <header className='w-full z-30 top-0 pt-3 pb-2 px-5 flex items-center justify-between fixed backdrop-blur-sm dark:bg-black/30'>
        ...
        <div className='text-sm flex items-center justify-center gap-x-3'>
          <a className={MENU_ITEM_CLASSES} href={`#${HOME_MENU.HOME}`}>
            Home
          </a>
          <a className={MENU_ITEM_CLASSES} href={`#${HOME_MENU.ABOUT}`}>
            About
          </a>
          <a className={MENU_ITEM_CLASSES} href={`#${HOME_MENU.TERMS}`}>
            Terms
          </a>
          {isAuthenticated ? (
            <a
              className={classnames(
                MENU_ITEM_CLASSES,
                'text-red-600 border-b-red-500 dark:border-b-red-500'
              )}
              href={ROUTES.CHAT}
            >
              START
            ...
            <a className={MENU_ITEM_CLASSES} href={`#${HOME_MENU.START}`}>
              Start
            </a>
          )}
        </div>
      ...
      <footer className='w-full mb-10 flex flex-col items-center justify-center z-30'>
        <div className='text-xs'>
          Developed by{' '}
          <a target='_blank' href='https://raminrezaei.ir'>
            Ramin
          </a>
        </div>
